...
public class DetectionService {
    ...
    // Helper method for adding reasons cleanly
    private void appendReason(StringBuilder reason, String text) {
    ...
    public int calculateRiskScore(Transaction transaction) {
        ...
        if (transaction.getAmount() > 50000) {
            score += 50;
            appendReason(reason, "High value transaction");
        }

        // RULE 2: Suspicious merchant
        if ("Unknown".equalsIgnoreCase(transaction.getMerchantName())) {
            score += 30;
            appendReason(reason, "Suspicious merchant");
        }
        ...
        try {
            ...
            if (hour >= 0 && hour <= 5) {
                score += 20;
                appendReason(reason, "Odd hour transaction");
            }
        ...
        if (userTransactions.size() >= 3) {
            score += 20;
            appendReason(reason, "Rapid multiple transactions");
        }
        ...
        if (!userTransactions.isEmpty()) {
            ...
            if (!lastLocation.equals(transaction.getLocation())) {
                score += 25;
                appendReason(reason, "Location mismatch");
            }
        }
    ...
    private int checkNewLocation(Transaction transaction,
        ...
        if (!knownLocation && !userTransactions.isEmpty()) {
            ...
            appendReason(reason, "New location detected");
